    ...
    <header class="header">
      ...
      <div class="navbar">
        <div class="navbar-left-container">
          <a href="/">
            <img class="navbar-brand-logo" src="static/logo1.png" width="140" height="50"></a>
        </div>
        <div class="navbar-right-container" style="display: flex;">
          <div class="navbar-menu-container">
            <!--<a href="/" class="navbar-link">我的账户</a>-->
            <span class="navbar-link" v-if="welcome">欢迎您{{ userName }}!</span>
            <a href="javascript:void(0)" class="navbar-link" @click="loginModalFlag=true" v-if="!welcome">Login</a>
            <a href="javascript:void(0)" class="navbar-link" v-if="welcome" @click="logout">Logout</a>
...
export default {
  ...
  methods: {
    async login() {
      ...
      let { data, status } = await this.axios.userLogin(param);
      if (status === 200) {
        alert(data.msg);
        if (data.status === 0) {
          this.welcome = true;
    ...
    async logout() {
      let { data, status } = await this.axios.userLogout();
      if (status === 200) {
        alert(data.msg);
        this.welcome = false;
      }
    ...
    async checkLogin() {
      let { data, status } = await this.axios.userCheckedLogin();
      if (status === 200) {
        // alert(data.msg);
        if (data.status === 1) {
          this.welcome = true;
