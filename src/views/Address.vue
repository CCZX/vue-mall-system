        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="page-title-normal checkout-title">
              <h2><span>购物地址栏</span></h2>
            </div>
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFliter" :key="item._id" @click="checkIndex = index, selectedAddrId = item.addressId" :class="{'check': checkIndex == index}">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                      <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(item.addressId)"><i>设置默认地址</i></a>
                    </div>
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" :to="{path: 'orderConfirm', query:{'addressId': selectedAddrId}}">结算</router-link>
            </div>
          </div>
        </div>
      </div>
...
import "./../assets/css/checkout.css";
  export default{
      ...
      methods: {
        async init() {
          let {data, status} = await this.axios.getAddressList()
          if (status == 200) {
        ...
        async setDefault(addressId) {
          let {data, status} = await this.axios.setDefault({'addressId': addressId})
          if (status == 200) {
            this.init()
          }
        },
        async delAddress(addressId) {
          let isDel = confirm("你确定要删除该地址吗?")
          if (!isDel) return
          let {data, status} = await this.axios.delAddress({'addressId': addressId})
          if (status == 200) {
            this.init()
          }
        }
