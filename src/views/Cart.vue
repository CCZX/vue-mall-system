          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:viod();"   @click.prevent="toggleCheckAll()">
                  <span class="checkbox-btn item-check-btn" :class="{'check': checkFlagAll}">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a class="btn btn--red" :class="{'btn--dis': this.checkedCount == 0}" @click="checkOut">Checkout</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    async init() {
      let { data, status } = await this.axios.getCartList();
      if (status === 200) {
    ...
    async delCart(productId) {
      let isDel = confirm("是否删除商品?");
      if (!isDel) return;
      let { data, status } = await this.axios.delCart({ productId: productId });
      if (status == 200) {
        alert(data.msg);
        if (data.status == 0) {
          this.cartList.forEach((item, index) => {
            // 遍历数组删除，不用this.init()减少二次请求
    ...
    async editCart(flag, item) {
      ...
      let { data, status } = await this.axios.editCart({
    ...
    async toggleCheckAll() {
      // this.checkFlagAll = !this.checkFlagAll
      let flag = !this.checkFlagAll;
      this.cartList.forEach(item => {
        item.checked = flag ? 1 : 0;
      });
      let { data, status } = await this.axios.toggleCheckedAll({
        checkAll: flag
      });
    },
    checkOut() {
